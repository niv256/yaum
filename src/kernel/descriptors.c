...
static void set_idt_entry (int index, uint32_t offset, uint16_t selector, uint8_t flags);
...
static idt_entry create_idt_entry(uint32_t offset, uint16_t selector, uint8_t flags);
...
gdt_entry gdt_entries[HARD_CODED_ENTRIES + SAVED_SPACE_FOR_ENTRIES];
...
void gdt_init(void){
	...
	gdt_entries[1] = create_gdt_entry(0, 0xFFFFF, 0x9A, 0xCF);	// kernel code
	gdt_entries[2] = create_gdt_entry(0, 0xFFFFF, 0x92, 0xCF);	// kernel data
	gdt_entries[3] = create_gdt_entry(0, 0xFFFFF, 0xFA, 0xCF);	// user code
	gdt_entries[4] = create_gdt_entry(0, 0xFFFFF, 0xF2, 0xCF);	// user data
...
void idt_init (void) {
	...
	// set all idt gates
	// selecor: kernel code in ring 0 (row 2) so 0x08
	// flags: gate type 0b1111 for 32bit interrupt, ring 0 DPL and not unused so 0x8E
	...
	set_idt_entry (34, (uint32_t) isr34, 0X08, 0X8E);
	set_idt_entry (35, (uint32_t) isr35, 0X08, 0X8E);
	set_idt_entry (36, (uint32_t) isr36, 0X08, 0X8E);
	set_idt_entry (37, (uint32_t) isr37, 0X08, 0X8E);
	set_idt_entry (38, (uint32_t) isr38, 0X08, 0X8E);
	set_idt_entry (39, (uint32_t) isr39, 0X08, 0X8E);
	set_idt_entry (40, (uint32_t) isr40, 0X08, 0X8E);
	set_idt_entry (41, (uint32_t) isr41, 0X08, 0X8E);
	set_idt_entry (42, (uint32_t) isr42, 0X08, 0X8E);
	set_idt_entry (43, (uint32_t) isr43, 0X08, 0X8E);
	set_idt_entry (44, (uint32_t) isr44, 0X08, 0X8E);
	set_idt_entry (45, (uint32_t) isr45, 0X08, 0X8E);
	set_idt_entry (46, (uint32_t) isr46, 0X08, 0X8E);
	set_idt_entry (47, (uint32_t) isr47, 0X08, 0X8E);
